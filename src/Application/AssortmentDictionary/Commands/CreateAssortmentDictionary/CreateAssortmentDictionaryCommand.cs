...
public record CreateAssortmentDictionaryCommand(string Number, string Name, string Distinguishing, string Unit, double? Amount, double? Weight, string? Material, string? Kind, ushort? DN1, ushort? DN2, ushort? Length, string RO, string? Comment) : IRequest
{
}
...
public class CreateAssortmentDictionaryCommandValidator : AbstractValidator<CreateAssortmentDictionaryCommand>
{
    public CreateAssortmentDictionaryCommandValidator()
    {
        ...
        RuleFor(x => x.Unit)
            ...
            .Must(x => x is Domain.Constants.Unit.KG or Domain.Constants.Unit.PIECE)
            .WithErrorCode("CREATE_ASSORTMENT_DICTIONARY_400_UNIT")
...
public class CreateAssortmentDictionaryCommandHandler : IRequestHandler<CreateAssortmentDictionaryCommand>
{
    ...
    {
        var (number, name, distinguishing, unit,  amount, weight,  material, kind, 
            dn1, dn2, length, ro, comment) = request;
        
        var assortmentDictionary = _factory.Create(Ulid.NewUlid(), number, name, distinguishing, unit,  amount, weight, material, kind, 
