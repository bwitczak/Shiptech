...
public record UpdateAssortmentDictionaryCommand(Ulid Id, string Number, string Name, string Distinguishing, string Unit, double? Amount, double? Weight, string? Material, string? Kind, ushort? DN1, ushort? DN2, ushort? Length, string RO, string? Comment) : IRequest
{
}
...
public class UpdateAssortmentDictionaryCommandHandler : IRequestHandler<UpdateAssortmentDictionaryCommand>
{
    ...
    {
        var (id, number, name, distinguishing, unit,  amount, weight,  material, kind, 
            dn1, dn2, length, ro, comment) = request;
        
        var updated = _factory.Create(id, number, name, distinguishing, unit,  amount, weight, material, kind, 
