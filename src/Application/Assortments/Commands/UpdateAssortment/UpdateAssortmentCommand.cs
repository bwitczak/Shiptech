...
namespace Shiptech.Application.Assortments.Commands.UpdateAssortment;

public record UpdateAssortmentCommand(
    Ulid Id,
    char Position,
...
public class UpdateAssortmentCommandValidator : AbstractValidator<UpdateAssortmentCommand>
{
    public UpdateAssortmentCommandValidator(IAssortmentService assortmentService, IIsoService isoService,
        IAssortmentDictionaryService assortmentDictionaryService)
    {
        RuleFor(x => x.Id)
            ...
            .MustAsync(async (x, _) => await assortmentService.ExistsById(x))
        ...
        RuleFor(x => x.PG)
            .NotNull()
            .NotEmpty()
            .WithErrorCode("UPDATE_ASSORTMENT_400_PG")
        ...
        When(x => x.ValveNumber is not null, () =>
        {
            RuleFor(x => x.ValveNumber)
                .NotNull()
                .NotEmpty()
                .WithErrorCode("UPDATE_ASSORTMENT_400_VALVE_NUMBER")
        ...
        When(x => x.CutAngle is not null, () =>
        {
            RuleFor(x => x.CutAngle)
                .NotNull()
                .NotEmpty()
                .WithErrorCode("UPDATE_ASSORTMENT_400_CUT_ANGLE")
        ...
        When(x => x.Comment is not null, () =>
        {
            RuleFor(x => x.Comment)
                .NotNull()
                .NotEmpty()
                .WithErrorCode("UPDATE_ASSORTMENT_400_COMMENT")
...
public class UpdateAssortmentCommandHandler : IRequestHandler<UpdateAssortmentCommand>
{
    private readonly IApplicationDbContext _context;
    private readonly IAssortmentFactory _factory;

    public UpdateAssortmentCommandHandler(IApplicationDbContext context, IAssortmentFactory factory)
    {
        _context = context;
        _factory = factory;
    }

    public async Task Handle(UpdateAssortmentCommand request, CancellationToken cancellationToken)
    {
        (Ulid id, char position, ushort prefabricationQuantity, ushort prefabricationLength,
            double prefabricationWeight, ushort assemblyQuantity, ushort assemblyLength, double assemblyWeight, char pg,
            string? valveNumber, string? cutAngle, string? comment, Ulid isoId, Ulid assortmentDictionaryId) = request;
        ...
        Assortment updated = _factory.Create(id, position, prefabricationQuantity, prefabricationLength,
        ...
        _context.Assortments.Update(updated);
