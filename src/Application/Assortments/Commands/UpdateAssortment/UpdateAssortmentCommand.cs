...
public record UpdateAssortmentCommand(Ulid Id, string Name, char Position, ushort? DrawingLength, ushort? Addition,
...
{
    public UpdateAssortmentCommandValidator(IAssortmentService assortmentService, IIsoService isoService, IAssortmentDictionaryService assortmentDictionaryService)
    {
        RuleFor(x => x.Id)
            .NotNull()
            .NotEmpty()
...
public class UpdateAssortmentCommandHandler : IRequestHandler<UpdateAssortmentCommand>
{
    ...
    public async Task Handle(UpdateAssortmentCommand request, CancellationToken cancellationToken)
    {
        ...
        var updated = _factory.Create(id, name, position, drawingLength, addition,
