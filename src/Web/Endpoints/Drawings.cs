...
using Shiptech.Application.Drawings.Queries.GetPaginatedDrawings;
...
public class Drawings : EndpointGroupBase
{
    ...
    {
        app.MapGroup(this)
            .MapGet(GetPaginatedDrawings, "/GetPaginated")
            .MapPost(CreateDrawing, "/Create")
            .MapPut(UpdateDrawing, "/Update/{id}")
            .MapDelete(DeleteDrawing, "/Delete/{id}");
    }

    private Task<PaginatedList<DrawingWithNoRelationsDto>> GetPaginatedDrawings(ISender sender, [AsParameters] GetPaginatedDrawingsQuery query)
    {
